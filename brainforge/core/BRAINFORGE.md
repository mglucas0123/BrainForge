...
- Read `.brainforge/memory/.context.md` and `.brainforge/memory/.user.md` now. **If `.context.md` is missing or empty, CREATE IT NOW** using the format in `MEMORY-FORMAT.md` — even for a brand-new/empty project. A minimal `§Stack: (unknown)§` entry is enough. Do NOT skip this step.
